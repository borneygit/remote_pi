  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    if (!_autoInited) {
      ...
      if (!isOneShotMode && cwd) {
        const trustAwareCtx = ctx as ExtensionContext & { isProjectTrusted?: () => boolean };
        ...
        if (canCreateLocalConfig && !localConfigExists(cwd)) {
          ...
          if (!localConfigExists(cwd)) {
            ctx.ui.notify(
              `[remote-pi] Could not auto-create ${cwd}/.pi/remote-pi/config.json; run /remote-pi setup to configure this project.`,
              "warning",
            );
          }
        }
