...
const FAILOVER_RETRY_BACKOFFS_MS = [100, 250, 500, 1_000, 2_000, 5_000];
...
export class SessionPeer {
    ...
    async _onSocketClose(closedSock) {
        ...
        while (!this.leftFlag && generation === this.reconnectGeneration) {
            const backoff = FAILOVER_RETRY_BACKOFFS_MS[Math.min(attempt, FAILOVER_RETRY_BACKOFFS_MS.length - 1)];
            await delay(backoff);
